...
namespace 单人剧情
{
	...
	CoTask<int> Co训练战(Space& refSpace, FunCancel& funCancel, std::shared_ptr<PlayerComponent> spPlayer可能空, const std::string strNickName)
	{
		...
		using namespace std;
		PlayerComponent::Say系统(spPlayer可能空, "欢迎来到RTS即时战略游戏，现在您要接受基础的训练");
		...
		//	co_return 0;
		//}
		refSpace.m_mapPlayer[strNickName].m_u32晶体矿 += 100;
		...
		单位::活动单位配置 配置;
		单位::Find活动单位配置(类型, 配置);
		SpEntity sp工程车 = refSpace.造活动单位(spPlayer可能空, strNickName, { 5,10 }, 配置, 类型);

		PlayerComponent::Say系统(spPlayer可能空, "请点击“工程车”选中，然后点击“造基地”按钮，再点击空白地面，10秒后就能造出一个基地");
		...
		PlayerComponent::Say系统(spPlayer可能空, "请点击选中基地（圆环特效表示选中），然后点击“造工程车”按钮，5秒后会在基地旁造出一个工程车");

		if (std::get<0>(co_await CoEvent<MyEvent::AddEntity>::Wait(funCancel, funSameSpace)))
			co_return 0;

		PlayerComponent::Say系统(spPlayer可能空, "现在您有工程车了，点击选中您的工程车，再点击空旷地面，命令它走向目标点");
		...
		PlayerComponent::Say系统(spPlayer可能空, "很好！现在给您刷了一个晶体矿，请点击晶体矿，让工程车在晶体矿和基地之间搬运晶体矿");
		...
		PlayerComponent::Say系统(spPlayer可能空, "很好！您的工程车正在采集晶体矿，请等他把晶体矿运回基地。现在，请选中另一辆工程车去采集燃气矿");
		if (std::get<0>(co_await CoEvent<MyEvent::晶体矿已运回基地>::Wait(funCancel)))
			co_return 0;

		PlayerComponent::Say系统(spPlayer可能空, "您的工程车已把第一车晶体矿运到基地，请查看左上角晶体矿数量变化");

		if (co_await CoTimer::Wait(5s, funCancel))
			co_return 0;

		PlayerComponent::Say系统(spPlayer可能空, "等您存够20晶体矿后，请选中一辆工程车，然后点击“造民房”按钮，再点击一次空旷地面");
		if (std::get<0>(co_await CoEvent<MyEvent::AddEntity>::Wait(funCancel, funSameSpace)))
			co_return 0;

		PlayerComponent::Say系统(spPlayer可能空, "很好，民房可以提升您的活动单位数量上限");

		if (co_await CoTimer::Wait(3s, funCancel))
			co_return 0;

		PlayerComponent::Say系统(spPlayer可能空, "等您存够30晶体矿后，请选中一辆工程车，然后点击“造兵厂”按钮，再点击一次空旷地面，就能造出一个兵厂");
		if (std::get<0>(co_await CoEvent<MyEvent::AddEntity>::Wait(funCancel, funSameSpace)))
			co_return 0;
		...
		PlayerComponent::Say系统(spPlayer可能空, "请点击选中兵厂（圆环特效表示选中），然后点击“造兵”按钮,10秒后会在兵厂旁造出一个兵");

		if (std::get<0>(co_await CoEvent<MyEvent::AddEntity>::Wait(funCancel, funSameSpace)))
			co_return 0;

		PlayerComponent::Say系统(spPlayer可能空, "鼠标点击选中您的兵，再点击地面，可以指挥他走向目标处");
		...
		PlayerComponent::Say系统(spPlayer可能空, "现在已在左边给您刷了一个怪，控制兵走到怪附近，兵会自动打怪。您可以点右下角“取消选中”然后拖动地面看看怪在哪里");
		...
		PlayerComponent::Say系统(spPlayer可能空, "您的兵阵亡了。可以多造点兵去围攻敌人");
		...
		PlayerComponent::Say系统(spPlayer可能空, "恭喜您消灭了敌人！现在左边给您刷了10个敌人。您可以造地堡,让兵进入地堡中，立足防守，再伺机进攻");
		...
		PlayerComponent::播放声音(spPlayer可能空, "音效/YouWin", "您取得了胜利！您真是指挥天才！");

		PlayerComponent::Send<Msg显示界面>(spPlayer可能空,{ .ui = Msg显示界面::选择地图 });
		co_return 0;
	}
	...
	CoTask<int> Co防守战(Space& refSpace,Entity &ref视口, FunCancel& funCancel, PlayerGateSession_Game& refGateSession)
	{
		...
		refSpace.m_mapPlayer[refGateSession.NickName()].m_u32燃气矿 += 1000;
		{
		...
		{
			{
				...
				单位::活动单位配置 配置;
				单位::Find活动单位配置(类型, 配置);
				refSpace.造活动单位(ref视口.m_spPlayer, refGateSession.NickName(), { -30, 30 }, 配置, 类型);
			}
