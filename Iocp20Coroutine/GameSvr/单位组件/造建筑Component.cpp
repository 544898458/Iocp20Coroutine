...
WpEntity 造建筑Component::AddBuilding(const 单位类型 类型, const Position pos)
{
	...
	auto wpNew = 创建建筑(m_refEntity.m_refSpace, pos, 类型, std::forward<UpPlayerComponent>(m_refEntity.m_upPlayer), EntitySystem::GetNickName(m_refEntity));
	PlayerComponent::Send资源(m_refEntity);
	return wpNew;
}
...
void 造建筑Component::根据建筑类型AddComponent(Space& refSpace, const 单位类型 类型, Entity& refNewEntity, UpPlayerComponent&& spPlayer, const std::string& strPlayerNickName)
{
	单位::建筑单位配置 建筑;
	单位::单位配置 单位;
	...
	if (虫巢 == 类型 || 太岁 == 类型)
	{
		auto wp苔蔓 = 造建筑Component::创建建筑(refSpace, refNewEntity.Pos(), 苔蔓, std::forward<UpPlayerComponent>(spPlayer), strPlayerNickName);
