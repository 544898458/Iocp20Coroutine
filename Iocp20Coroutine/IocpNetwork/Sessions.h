...
public:
	...
	void ForEach(const std::function<void(T_Session&)>& fun);
private:
...
template<class T_Session>
void Sessions<T_Session>::ForEach(const std::function<void(T_Session&)>& fun)
{
	std::lock_guard lock(m_setSessionMutex);
	for (auto& [k, pSession] : m_mapSession)
	{
		fun(pSession->Session);
	}
}
