...
CoTaskBool 采集Component::Co采集(PlayerGateSession_Game& refGateSession, Entity& refThis, WpEntity wp目标资源)
{
	...
	while (true)
	{
		...
		{
			co_await CoTimer::Wait(1s, m_funCancel);//自己连一个基地都没有，等一会儿再试
			continue;
		}
		...
		if (Max携带晶体矿() <= m_u32携带晶体矿)//装满了，回基地放矿
		{
			...
			{
				if (co_await CoTimer::Wait(1s, m_funCancel))//把矿放进基地耗时
					co_return true;
			...
			//离基地太远，走向基地
			if (co_await AiCo::WalkToTarget(refThis.shared_from_this(), wpEntity基地.lock(), m_funCancel, false))
				co_return true;
		...
		//距离目标矿太远，走向晶体矿
		if (co_await AiCo::WalkToTarget(refThis.shared_from_this(), wp目标资源.lock(), m_funCancel, false))
			co_return true;
	}
}
